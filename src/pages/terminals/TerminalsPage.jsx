...
export default function TerminalsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setRows(Array.isArray(termRes?.data?.terminals) ? termRes.data.terminals : []);
      } catch {
        if (!ignore) {
          setRows([]);
        }
  ...
  const terminalMeta = useMemo(() => {
    ...
    (rows ?? []).forEach((t) => {
      ...
      map.set(tid, {
        ...
        mid,
        type: terminalType,
        location: String(t?.Location ?? t?.location ?? t?.Branch ?? t?.branch ?? t?.serial_number ?? "").trim(),
        active: isActive,
      });
    });
    return map;
  ...
  const openEdit = (row) => {
    const tid = String(row?.TID ?? row?.TerminalID ?? row?.terminalId ?? "").trim();
    const meta = terminalMeta.get(tid);
    const active = Boolean(meta?.active);
    const mid = String(row?.MID ?? row?.MerchantID ?? row?.merchantId ?? meta?.mid ?? "").trim();

    setEditTid(tid);
    setEditForm({
      tid,
      mid,
      type: String(row?.posType ?? row?.POSType ?? row?.type ?? row?.Type ?? meta?.type ?? "").trim(),
      serialNumber: String(row?.serial_number ?? row?.SerialNumber ?? row?.serialNumber ?? "").trim(),
  ...
  const filteredRows = useMemo(() => {
    ...
    return (rows ?? []).filter((t) => {
      ...
      if (!q) return true;
      return [tid, mid, type].some((v) => String(v ?? "").toLowerCase().includes(q));
    });
  }, [rows, searchValue, statusFilter, terminalMeta]);

  const exportCsv = () => {
    const headers = ["TerminalID", "MerchantID", "Type", "SerialNumber", "PhoneNumber", "Status"];
    const lines = [headers.join(",")];

    filteredRows.forEach((t) => {
      const tid = String(t?.TID ?? t?.TerminalID ?? t?.terminalId ?? "").trim();
      const mid = String(t?.MID ?? t?.MerchantID ?? t?.merchantId ?? "").trim();
      const type = String(t?.posType ?? t?.POSType ?? t?.type ?? t?.Type ?? "").trim();
      ...
      lines.push([tid, mid, JSON.stringify(type), JSON.stringify(serial), JSON.stringify(phone), status].join(","));
    });
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-8">
      <div className="mb-5 text-xs text-slate-500">
        Operations / Terminal / <span className="text-sky-400">All/Modify Terminal</span>
      </div>

      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-100">All/Modify Terminal</h1>
          <p className="mt-1 text-sm text-slate-400">Edit, activate or deactivate terminals</p>
        </div>
        ...
        <div className="relative flex flex-wrap items-end gap-3">
          <div className="min-w-[280px] flex-1">
            <div className="relative">
              <i className="pi pi-search absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-500" aria-hidden />
              <InputText
                value={searchValue}
      ...
      <section className="mt-5 relative overflow-hidden rounded-2xl border border-white/5 bg-[#0b1220]/70 p-5 shadow-[0_10px_30px_rgba(0,0,0,0.35)] backdrop-blur">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-black/20">
          ...
          >
            ...
            <Column
              header="Merchant ID"
              body={(row) => String(row?.MID ?? row?.MerchantID ?? row?.merchantId ?? "-")}
              headerClassName="!border-0 !bg-transparent px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
              bodyClassName="px-4 py-3 !border-0 border-t border-white/5 text-sm text-slate-200"
            />
            <Column
              header="Type"
              body={(row) => String(row?.posType ?? row?.POSType ?? row?.type ?? row?.Type ?? "-")}
              headerClassName="!border-0 !bg-transparent px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
              bodyClassName="px-4 py-3 !border-0 border-t border-white/5 text-sm text-slate-200"
            />
            <Column
              header="Serial Number"
              body={(row) => String(row?.serial_number ?? row?.SerialNumber ?? "-")}
              headerClassName="!border-0 !bg-transparent px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
              bodyClassName="px-4 py-3 !border-0 border-t border-white/5 text-sm text-slate-200"
            />
            <Column
              header="Phone Number"
              body={(row) => String(row?.PhoneNumber ?? row?.phoneNumber ?? "-")}
              headerClassName="!border-0 !bg-transparent px-4 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400"
              bodyClassName="px-4 py-3 !border-0 border-t border-white/5 text-sm text-slate-200"
            />
            <Column
                ...
                return (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={() => openConfirm(tid, active ? "deactivate" : "activate")}
                    >
      ...
      >
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b1220] text-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
          ...
          <div className="relative p-6">
            ...
            <div>
              <div className="text-base font-semibold tracking-tight">Edit Terminal</div>
              <div className="mt-1 text-xs text-slate-400">ID: {editForm.tid || "-"}</div>
            </div>
            <button
              type="button"
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
              <label className="mb-2 block text-xs font-medium text-slate-300">Terminal ID</label>
              <InputText
                value={editForm.tid}
                disabled
            ...
            <div>
              ...
              <InputText
                value={editForm.mid}
                onChange={(e) => setEditForm((prev) => ({ ...prev, mid: e.target.value }))}
            ...
            <div>
              <label className="mb-2 block text-xs font-medium text-slate-300">Type</label>
              <Dropdown
                value={editForm.type}
                options={typeOptions}
            ...
            <div>
              <label className="mb-2 block text-xs font-medium text-slate-300">Serial Number</label>
              <InputText
        ...
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b1220] text-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
          ...
          <div className="relative p-7">
            <div className="text-lg font-semibold tracking-tight">
              {confirmAction === "activate" ? "Activate Terminal" : "Deactivate Terminal"}
            </div>
            <div className="mt-2 text-sm text-slate-400">
              You are about to change the status of: <span className="text-slate-200">{confirmTid || "-"}</span>
            </div>
            ...
            <div className="mt-6 border-t border-white/10 pt-6 flex items-center justify-end gap-3">
              ...
              <Button
                type="button"
                label={confirmAction === "activate" ? "Activate Terminal" : "Deactivate Terminal"}
