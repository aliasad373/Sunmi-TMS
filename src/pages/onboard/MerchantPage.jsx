...
export default function MerchantPage() {
  ...
  const [selectedMerchant1, setSelectedMerchant1] = useState(null);
  ...
  const filteredRows = useMemo(() => {
    ...
    return (rows ?? []).filter((m) => {
      ...
      return [mid, merchantName, businessName, phone, address].some((v) =>
  ...
  const getMid = useCallback(
    (row) =>
      String(row?.MID ?? row?.MerchantID ?? row?.merchantId ?? "").trim(),
    [],
  );
  ...
  const confirmDelete = async () => {
    if (!selectedMerchant1?.dbId) {
      console.log("Selected merchant:", selectedMerchant1);
      alert("Merchant ID not found");
      return;
    }

    try {
      console.log("Deleting:", selectedMerchant1.dbId);

      await api.delete(`/delete-merchant/${selectedMerchant1.dbId}`);

      setDeleteOpen(false);
      setSelectedMerchant1(null);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-8">
      <div className="mb-5 text-xs text-slate-500">
        Operations / Merchant /{" "}
        <span className="text-sky-400">Modify Merchants</span>
      </div>
      ...
      >
        <div className="text-sm text-slate-300">
          ...
          {confirmNextActive ? "activate" : "deactivate"} this merchant?
        </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <div>
                ...
                <Dropdown
                  ...
                  onChange={(e) =>
                    setEditForm((p) => ({ ...p, status: e.value }))
                  }
              ...
              <div>
                ...
                <InputText
                  ...
                  onChange={(e) =>
                    setEditForm((p) => ({ ...p, merchantName: e.target.value }))
                  }
              ...
              <div>
                ...
                <Dropdown
                  ...
                  onChange={(e) =>
                    setEditForm((p) => ({ ...p, businessType: e.value }))
                  }
      ...
      >
        <div className="rounded-2xl border border-white/10 bg-[#0b1220] p-6 text-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold">Delete Merchant</h3>
